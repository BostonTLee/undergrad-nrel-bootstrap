...
This paper looks at data from a 12 year period to determine the current 
output of a specified city. It can be used to determine 
on a month by month basis if a location produces a sufficient amount 
of current from a solar panel. Providing customers with an idea on 
whether or not they should make the investment that is solar.
...
estimates for average (simulated) current output
...
The current output provides a good test case for this.
Both the preprocessed and postprocessed currents are not independent data;
values of either irradiance or current output
...
For our purposes, $\theta$ is simply the average current output
...
the daytime current values for each day.
Given that our daytime current values for each day
...
```{r current-ts, fig.cap="Time series of simulated current values for the first six days in the dataset.", echo=FALSE}
print(time_series_current)
```
...
```{r current-hour-boxplot, fig.cap="Distribution of GHI at each hour for the entire unfiltered dataset.", echo=FALSE}
print(hour_plot_current)
```
...
```{r mean-boot-plot, fig.cap="Distributions of bootstrap mean simulated current for both iid and block bootstrapping methods. Percentile 95 percent bootstrap confidence intervals for mean simulated current overlayed.", echo=FALSE}
...
caption="95% confidence intervals for the mean simulated current, as generated by each of the two bootstrapping methods.")
